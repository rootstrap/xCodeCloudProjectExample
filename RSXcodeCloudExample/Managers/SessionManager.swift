...
internal class SessionManager: CurrentUserSessionProvider {
  ...
  var currentSession: Session? {
    get {
      if
        let data = userDefaults.data(forKey: "RSXcodeCloudExample-session"),
    ...
    set {
      ...
      userDefaults.set(session, forKey: "RSXcodeCloudExample-session")
    }
  }
  
  func deleteSession() {
    userDefaults.removeObject(forKey: "RSXcodeCloudExample-session")
  }
